...
public:
  ...
  {
      this->Rows = new ROW[other.NumRows];  // an array with the same number of ROW structs as other
      this->NumRows = other.NumRows;

      // initialize each row to have same num of columns as other:
      for (int r = 0; r < NumRows; ++r)
      {
          this->Rows[r].Cols = new T[other.Rows[r].NumCols];  // an array with elements of type T:
          this->Rows[r].NumCols = other.Rows[r].NumCols;

          // initialize the elements to the same value as other:
          for (int c = 0; c < Rows[r].NumCols; ++c)
          {
              this->Rows[r].Cols[c] = other.Rows[r].Cols[c];  // other value for type T:
          }
      }
  }
  ...
  int numcols(int r) const
  {
    ...
    return Rows[r].NumCols; //number of rows in column r-1
  }
  ...
  void growcols(int r, int C)
  {
    ...
    //creates a matrix with one row and sets copies values of first old columns to new columns
    if(Rows[r].NumCols < C){
        mymatrix<T> newMatrix(1, C);
        for(int i = 0; i < Rows[r].NumCols; i++){
            newMatrix.Rows[0].Cols[i] = Rows[r].Cols[i];
        }

        //change num cols and make sure extras initialized to T
        Rows[r] = newMatrix.Rows[0];
        Rows[r].NumCols = C;
  ...
  void grow(int R, int C)
  {
    ...
    //grows the columns to C
    for(int i = 0; i < NumRows; i++){
        growcols(i, C);
    }

    if(NumRows < R) {
        mymatrix<T> newMatrix(R, C); //creates parameterized matrix with new dimensions
        for (int r = 0; r < NumRows; r++) {
            for (int c = 0; c < Rows->NumCols; c++) {
                newMatrix.Rows[r].Cols[c] = Rows[r].Cols[c]; //copies values from old matrix
            }
        }

        Rows = newMatrix.Rows;

        // Sets number of columns for new rows
        for (int r = NumRows; r < R; r++) {
            Rows[r].NumCols = C;
            for (int c = 0; c < C; c++) {
                Rows[r].Cols[c] = T(); // Initializes the element to its default values
            }
        }
  ...
  int size() const
  {
    ...
    // goes through matrix rows and adds the column count
    int total = 0;
    for(int r = 0; r < NumRows; r++){
  ...
  {
    mymatrix<T> result(*this); // creates copy of current matrix

    // goes through copy matrix and multiplies each value by scalar
    for(int r = 0; r < NumRows; r++){
        for(int c = 0; c < Rows[r].NumCols; c++){
  ...
  // checks if the matrix is rectangular and returns bool value
  // if one of the rows number of columns does not match the rest, returns false
  bool is_rectangular() const{
      int columns = Rows[0].NumCols;
      for(int i = 0; i < NumRows; i++){
          if(Rows[i].NumCols != columns){
              return false;
          }
      }
      return true;
  ...
  mymatrix<T> operator*(const mymatrix<T>& other)
  {
    ...
    // creates a new parameterized matrix with size of resulting matrix by multiplication
    mymatrix<T> result(NumRows, other.Rows[0].NumCols);
      ...
      for(int i = 0; i < NumRows; i++){
          for(int j = 0; j < other.Rows[0].NumCols; j++){
              ...
              for(int inner = 0; inner < Rows[0].NumCols; inner++){
                  total += (Rows[i].Cols[inner] * other.Rows[inner].Cols[j]);
              }
